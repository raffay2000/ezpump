...
class Account extends Component{
    ...
    constructor(props){
     ...
     componentDidMount(){
        this.callData();
        console.log("review",this.state.user?.get_reviews)
     }
    callData = async()=>{
    ...
    //     </View>
    // );
    // ReviewObj = this.state.user?.get_reviews;

        
    
    renderContent = () => {
        const {user_type, description, phone_number, email, website, address, created_at, user_name, points} = this.state?.user;
        console.log("raffay",user_type);
        return (
                    ...
                    {/* {this.ReviewObj.length>0 &&
                        this.ReviewObj.map((item, index)=>{
                            return(
                                <Reviews
                                key={item.id}
                        NameTxt={item?.get_reviewer?.user_name}
                        Review={item?.review}
