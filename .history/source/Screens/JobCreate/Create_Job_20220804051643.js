...
class Create_Job extends Component{
    ...
    onAcceptPress = () => {
        ...
        const {Add_1, Add_2, Description , title, Line, time, date, volume, map, stateValue, priceFrom, priceTo, pumpType, jobType, } = this.state; 
        const {latitude, longitude} = this.props.route.params.region;
        ...
        // return console.log(typeof 'asd asd'.split(' '))
            ...
            data.append('lat', latitude); //changes required in Api Here
            data.append('lng', longitude); //changes required in Api Here
    ...
    onPostJobPress = () => {
        const {Add_1, Add_2, title, Description, Line, time, date, volume, map, stateValue, priceFrom, priceTo, pumpType, jobType, } = this.state;
        const {lat, lng} = map;
        if(Add_1 && Add_2 && Description && title && Line && time && date && volume && 
            // map && 
            stateValue && priceFrom && priceTo
            //  && pumpType 
            //  && jobType
              ){
    ...
    render(){
        const {loading, Add_1, Add_2, title, Description, Line, mode, date, time, volume, jobType, pumpType, showDatePicker, termsModal, priceFrom, priceTo, stateValue } = this.state;
            ...
            <>
                ...
                <ScrollView showsVerticalScrollIndicator={false} nestedScrollEnabled={true} >
                    <View style={styles.main}>
                        ...
                        />
                         <View style={styles.Mid_Heading} >
                            <Text style={[styles.Txt, {color:white, fontSize:hp("1.6%"), lineHeight:hp("1.7%"), fontFamily:semiBold}]} >Job Title</Text>
                        </View>
                        <TextInput
                            value={title}
                        ...
                        <View style={[styles.miniContainer,{marginTop:hp("1%"), paddingLeft:hp("1%"), paddingRight:hp("1%")}]} >
                            <Text style={[styles.Txt, {color:this.props.route.params?.desp ?"#000":"#979797"}]} >{this.props.route.params?.desp ? this.props.route.params?.desp :"Map" }</Text>
                        ...
                        <View style={[styles.miniContainer,{marginTop:hp("2%"), paddingLeft:hp("1%"), paddingRight:hp("1%")}]} >
                            ...
                            <TouchableOpacity onPress={this.showDatePicker} style={{flexDirection:"row", alignItems:"center", justifyContent:"flex-end"}} >
                                ...
                                <Ionicons name="calendar" color={"#000"} size={hp("3%")} style={{marginLeft:hp("1.5%")}}/>
                            </TouchableOpacity>
                        </View>
                        <View style={[styles.miniContainer,{marginTop:hp("2%"), paddingLeft:hp("1%"), paddingRight:hp("1%")}]} >
