...
class ApplyJob extends Component{
    ...
    onSendButtonPress = () => {
        ...
        if(bidAmount && proposal){
            if(bidAmount > price_to || bidAmount < price_from) {
                return Toast.show({text1: "Bid Amount should be with in bid range"})
            }
            this.setState({confirmModal: true})
        }else{
            Toast.show({text1: "Enter Proposal and Bid Amount"})
        }
    }
    render() {
                ...
                <ScrollView showsVerticalScrollIndicator={false} >
                    ...
                    {!isPending
                        ...
                        <>
                            ...
                            <View style={styles.miniContainer} >
                                <Text style={[styles.Txt, {fontFamily:bold, fontSize:hp('2.5%'), }]} >{user.points}</Text>
