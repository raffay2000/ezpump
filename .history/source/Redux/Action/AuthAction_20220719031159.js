    ...
    return function(dispatch, getState){
        ...
        .then(function (response) {
            ...
            registerStepOneDone(dispatch, "Registration Successful")
                return navigate('Register',{email, password,phone, userType})
            }else{
            }
        })
        .catch(function (error) {
...
export const registerStepTwo = (email,password,phone5,userType,website,ImgSrc,is_agree) => {
    return function(dispatch, getState){
        const {name,username,description} = getState().authReducer;
        console.log("email",email,"password", password,"phone", phone5,"CompanyName",name,"userName",username,"description",description,"website",website,"is_agree",is_agree)
