...
export const registerStepOne = () => {
    return function(dispatch, getState){
        ...
        console.log(email, password, phone, userType,isCheck)
        dispatch({
        ...
        .then(function (response) {
            ...
            registerStepOneDone(dispatch, "Registration Successful")
                return navigate('Register',{email,password,phone})
            }else{
            }
        })
        .catch(function (error) {
...
export const registerStepTwo = (ImgSrc) => {
    return function(dispatch, getState){
        const {name,username,description,email,password,phone,isAgree,website,userType} = getState().authReducer;
        console.log("this is in redux","email",email,"password", password,"phone", phone,"CompanyName",name,"userName",username,"description",description,"website",website,"is_agree",isAgree,"userType",userType)
        dispatch({
        ...
        data.append('is_agree', isAgree ? '1' : '0');
